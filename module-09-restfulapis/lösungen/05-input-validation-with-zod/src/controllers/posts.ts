...
const createPost: RequestHandler<{}, PostDTO, PostInputDTO> = async (
  ...
  const populatedPost = await post.populate<{ userId: PopulatedUserDTO }>(
    "userId",
...
const getPostById: RequestHandler<IdParams, PostDTO> = async (req, res) => {
  ...
  const post = await Post.findById(id).populate<{ userId: PopulatedUserDTO }>(
    "userId",
    "firstName lastName email",
  );
...
const updatePost: RequestHandler<IdParams, PostDTO, PostInputDTO> = async (
  req,
  res,
) => {
  const {
...
const deletePost: RequestHandler<IdParams, { message: string }> = async (
  req,
  res,
) => {
  const {
